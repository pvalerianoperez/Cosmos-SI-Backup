
using System;
using System.Data;
using System.Collections.Generic;
using Cosmos.Framework;

namespace Cosmos.Sistema.Entidades
{
    public class EstatusProducto
    {        
        public int EstatusProductoID { get; set; }            
        public string EstatusProductoClave { get; set; }            
        public string Nombre { get; set; }            
        public string NombreCorto { get; set; }            
           
        public void Load(DataRow dr){
            if(dr!=null)            
            {
                String nombreColumna="";
                String prefijoTabla = "SistemaEstatusProducto";
                
                //EstatusProductoID
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "EstatusProductoID", prefijoTabla);
                if(!nombreColumna.Equals("")){this.EstatusProductoID =  CastHelper.CInt2(dr[nombreColumna]);}   
                
                //EstatusProductoClave
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "EstatusProductoClave", prefijoTabla);
                if(!nombreColumna.Equals("")){this.EstatusProductoClave =  CastHelper.CStr2(dr[nombreColumna]);}   
                
                //Nombre
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "Nombre", prefijoTabla);
                if(!nombreColumna.Equals("")){this.Nombre =  CastHelper.CStr2(dr[nombreColumna]);}   
                
                //NombreCorto
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "NombreCorto", prefijoTabla);
                if(!nombreColumna.Equals("")){this.NombreCorto =  CastHelper.CStr2(dr[nombreColumna]);}   
                
            }
        }
    }
}
