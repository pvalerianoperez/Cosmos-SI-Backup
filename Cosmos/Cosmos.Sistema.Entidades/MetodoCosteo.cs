
using System;
using System.Data;
using System.Collections.Generic;
using Cosmos.Framework;

namespace Cosmos.Sistema.Entidades
{
    public class MetodoCosteo
    {        
        public int MetodoCosteoID { get; set; }            
        public string MetodoCosteoClave { get; set; }            
        public string Nombre { get; set; }            
        public string NombreCorto { get; set; }            
           
        public void Load(DataRow dr){
            if(dr!=null)            
            {
                String nombreColumna="";
                String prefijoTabla = "SistemaMetodoCosteo";
                
                //MetodoCosteoID
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "MetodoCosteoID", prefijoTabla);
                if(!nombreColumna.Equals("")){this.MetodoCosteoID =  CastHelper.CInt2(dr[nombreColumna]);}   
                
                //MetodoCosteoClave
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "MetodoCosteoClave", prefijoTabla);
                if(!nombreColumna.Equals("")){this.MetodoCosteoClave =  CastHelper.CStr2(dr[nombreColumna]);}   
                
                //Nombre
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "Nombre", prefijoTabla);
                if(!nombreColumna.Equals("")){this.Nombre =  CastHelper.CStr2(dr[nombreColumna]);}   
                
                //NombreCorto
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "NombreCorto", prefijoTabla);
                if(!nombreColumna.Equals("")){this.NombreCorto =  CastHelper.CStr2(dr[nombreColumna]);}   
                
            }
        }
    }
}
