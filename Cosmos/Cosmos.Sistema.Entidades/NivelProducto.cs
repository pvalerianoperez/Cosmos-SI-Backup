
using System;
using System.Data;
using System.Collections.Generic;
using Cosmos.Framework;

namespace Cosmos.Sistema.Entidades
{
    public class NivelProducto
    {        
        public int NivelProductoID { get; set; }            
        public string NivelProductoClave { get; set; }            
        public string Nombre { get; set; }            
        public string NombreCorto { get; set; }            
           
        public void Load(DataRow dr){
            if(dr!=null)            
            {
                String nombreColumna="";
                String prefijoTabla = "SistemaNivelProducto";
                
                //NivelProductoID
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "NivelProductoID", prefijoTabla);
                if(!nombreColumna.Equals("")){this.NivelProductoID =  CastHelper.CInt2(dr[nombreColumna]);}   
                
                //NivelProductoClave
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "NivelProductoClave", prefijoTabla);
                if(!nombreColumna.Equals("")){this.NivelProductoClave =  CastHelper.CStr2(dr[nombreColumna]);}   
                
                //Nombre
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "Nombre", prefijoTabla);
                if(!nombreColumna.Equals("")){this.Nombre =  CastHelper.CStr2(dr[nombreColumna]);}   
                
                //NombreCorto
                nombreColumna = EntidadesHelper.ColumnaDatosPropiedadEntidad(dr, "NombreCorto", prefijoTabla);
                if(!nombreColumna.Equals("")){this.NombreCorto =  CastHelper.CStr2(dr[nombreColumna]);}   
                
            }
        }
    }
}
