        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Almacen_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Almacen_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string almacenClave = CastHelper.CStr2(e.InsertValues[i].NewValues["AlmacenClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Almacen.Guardar(almacenID, almacenClave, nombre, nombreCorto, sucursalID);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int almacenID = CastHelper.CInt2(e.UpdateValues[i].Keys["AlmacenID"]);
                    string almacenClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["AlmacenClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int almacenID = CastHelper.CInt2(e.DeleteValues[i].Values["AlmacenID"]);
                    Cosmos.Compras.Api.Client.Almacen.Eliminar(almacenID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
