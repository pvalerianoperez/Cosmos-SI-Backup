...
{
    public partial class Area_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Area_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Area_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string areaClave = CastHelper.CStr2(e.InsertValues[i].NewValues["AreaClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Area.Guardar(areaId, empresaID,  areaClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int areaId = CastHelper.CInt2(e.UpdateValues[i].Keys["AreaId"]);
                    string areaClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["AreaClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int areaId = CastHelper.CInt2(e.DeleteValues[i].Values["AreaId"]);
                    Cosmos.Compras.Api.Client.Area.Eliminar(areaId);
                }
            }
            e.Handled = true;
            Grid.DataBind();
