...
{
    public partial class Banco_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Banco_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Banco_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int bancoID = CastHelper.CInt2(e.InsertValues[i].NewValues["BancoID"]);
                    string bancoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["BancoClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Banco.Guardar(bancoID, bancoClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int bancoID = CastHelper.CInt2(e.UpdateValues[i].Keys["BancoID"]);
                    string bancoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["BancoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int bancoID = CastHelper.CInt2(e.DeleteValues[i].Values["BancoID"]);
                    Cosmos.Compras.Api.Client.Banco.Eliminar(bancoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
