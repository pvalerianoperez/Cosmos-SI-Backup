...
{
    public partial class ConceptoIngreso_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("ConceptoIngreso_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("ConceptoIngreso_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int conceptoIngresoID = CastHelper.CInt2(e.InsertValues[i].NewValues["ConceptoIngresoID"]);
                    ...
                    Cosmos.Compras.Api.Client.ConceptoIngreso.Guardar(conceptoIngresoID, nombre, nombreCorto, empresaID);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int conceptoIngresoID = CastHelper.CInt2(e.UpdateValues[i].Keys["ConceptoIngresoID"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int conceptoIngresoID = CastHelper.CInt2(e.DeleteValues[i].Values["ConceptoIngresoID"]);
                    Cosmos.Compras.Api.Client.ConceptoIngreso.Eliminar(conceptoIngresoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
