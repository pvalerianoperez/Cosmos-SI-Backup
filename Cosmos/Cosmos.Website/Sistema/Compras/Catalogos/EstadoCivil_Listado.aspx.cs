...
{
    public partial class EstadoCivil_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("EstadoCivil_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("EstadoCivil_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string estadoCivilClave = CastHelper.CStr2(e.InsertValues[i].NewValues["EstadoCivilClave"]);
                    ...
                    Cosmos.Compras.Api.Client.EstadoCivil.Guardar(estadoCivilID, estadoCivilClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int estadoCivilID = CastHelper.CInt2(e.UpdateValues[i].Keys["EstadoCivilID"]);
                    string estadoCivilClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["EstadoCivilClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int estadoCivilID = CastHelper.CInt2(e.DeleteValues[i].Values["EstadoCivilID"]);
                    Cosmos.Compras.Api.Client.EstadoCivil.Eliminar(estadoCivilID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
