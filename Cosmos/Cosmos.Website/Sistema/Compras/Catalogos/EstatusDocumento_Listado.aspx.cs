        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("EstatusDocumento_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("EstatusDocumento_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string estatusDocumentoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["EstatusDocumentoClave"]);
                    ...
                    Cosmos.Compras.Api.Client.EstatusDocumento.Guardar(estatusDocumentoID, estatusDocumentoClave, nombre, nombreCorto, sistemaEstatusTipoDocumentoID, predeterminado);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int estatusDocumentoID = CastHelper.CInt2(e.UpdateValues[i].Keys["EstatusDocumentoID"]);
                    string estatusDocumentoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["EstatusDocumentoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int estatusDocumentoID = CastHelper.CInt2(e.DeleteValues[i].Values["EstatusDocumentoID"]);
                    Cosmos.Compras.Api.Client.EstatusDocumento.Eliminar(estatusDocumentoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
