    ...
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("FormaPago_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("FormaPago_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int formaPagoID = CastHelper.CInt2(e.InsertValues[i].NewValues["FormaPagoID"]);
                    ...
                    Cosmos.Compras.Api.Client.FormaPago.Guardar(formaPagoID, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int formaPagoID = CastHelper.CInt2(e.UpdateValues[i].Keys["FormaPagoID"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int formaPagoID = CastHelper.CInt2(e.DeleteValues[i].Values["FormaPagoID"]);
                    Cosmos.Compras.Api.Client.FormaPago.Eliminar(formaPagoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
