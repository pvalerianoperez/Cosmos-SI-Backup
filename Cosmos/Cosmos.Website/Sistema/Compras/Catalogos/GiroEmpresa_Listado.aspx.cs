...
{
    public partial class GiroEmpresa_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("GiroEmpresa_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("GiroEmpresa_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int giroEmpresaID = CastHelper.CInt2(e.InsertValues[i].NewValues["GiroEmpresaID"]);
                    string giroEmpresaClave = CastHelper.CStr2(e.InsertValues[i].NewValues["GiroEmpresaClave"]);
                    ...
                    Cosmos.Compras.Api.Client.GiroEmpresa.Guardar(giroEmpresaID, giroEmpresaClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int giroEmpresaID = CastHelper.CInt2(e.UpdateValues[i].Keys["GiroEmpresaID"]);
                    string giroEmpresaClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["GiroEmpresaClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int giroEmpresaID = CastHelper.CInt2(e.DeleteValues[i].Values["GiroEmpresaID"]);
                    Cosmos.Compras.Api.Client.GiroEmpresa.Eliminar(giroEmpresaID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
