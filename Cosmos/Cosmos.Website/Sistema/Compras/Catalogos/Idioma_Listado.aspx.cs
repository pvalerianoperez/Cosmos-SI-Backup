...
{
    public partial class Idioma_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Idioma_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Idioma_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int idiomaID = CastHelper.CInt2(e.InsertValues[i].NewValues["IdiomaID"]);
                    ...
                    Cosmos.Compras.Api.Client.Idioma.Guardar(idiomaID, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int idiomaID = CastHelper.CInt2(e.UpdateValues[i].Keys["IdiomaID"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int idiomaID = CastHelper.CInt2(e.DeleteValues[i].Values["IdiomaID"]);
                    Cosmos.Compras.Api.Client.Idioma.Eliminar(idiomaID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
