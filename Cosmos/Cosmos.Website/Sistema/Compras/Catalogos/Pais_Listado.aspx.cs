...
{
    public partial class Pais_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Pais_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Pais_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int paisID = CastHelper.CInt2(e.InsertValues[i].NewValues["PaisID"]);
                    string paisClave = CastHelper.CStr2(e.InsertValues[i].NewValues["PaisClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Pais.Guardar(paisID, paisClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int paisID = CastHelper.CInt2(e.UpdateValues[i].Keys["PaisID"]);
                    string paisClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["PaisClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int paisID = CastHelper.CInt2(e.DeleteValues[i].Values["PaisID"]);
                    Cosmos.Compras.Api.Client.Pais.Eliminar(paisID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
