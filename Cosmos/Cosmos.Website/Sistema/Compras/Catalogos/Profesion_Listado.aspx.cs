...
{
    public partial class Profesion_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Profesion_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Profesion_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int profesionID = CastHelper.CInt2(e.InsertValues[i].NewValues["ProfesionID"]);
                    string profesionClave = CastHelper.CStr2(e.InsertValues[i].NewValues["ProfesionClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Profesion.Guardar(profesionID, profesionClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int profesionID = CastHelper.CInt2(e.UpdateValues[i].Keys["ProfesionID"]);
                    string profesionClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["ProfesionClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int profesionID = CastHelper.CInt2(e.DeleteValues[i].Values["ProfesionID"]);
                    Cosmos.Compras.Api.Client.Profesion.Eliminar(profesionID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
