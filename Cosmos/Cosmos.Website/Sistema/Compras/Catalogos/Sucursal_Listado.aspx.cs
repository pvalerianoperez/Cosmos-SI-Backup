...
{
    public partial class Sucursal_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Sucursal_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Sucursal_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            }
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string sucursalClave = CastHelper.CStr2(e.InsertValues[i].NewValues["SucursalClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Sucursal.Guardar(sucursalID, sucursalClave, nombre, nombreCorto, empresaID, domicilioID);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int sucursalID = CastHelper.CInt2(e.UpdateValues[i].Keys["SucursalID"]);
                    string sucursalClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["SucursalClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int sucursalID = CastHelper.CInt2(e.DeleteValues[i].Values["SucursalID"]);
                    Cosmos.Compras.Api.Client.Sucursal.Eliminar(sucursalID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
