...
{
    public partial class TipoProducto_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("TipoProducto_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("TipoProducto_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string tipoProductoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["TipoProductoClave"]);
                    ...
                    Cosmos.Compras.Api.Client.TipoProducto.Guardar(tipoProductoID, tipoProductoClave, nombre, nombreCorto, empresaID);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int tipoProductoID = CastHelper.CInt2(e.UpdateValues[i].Keys["TipoProductoID"]);
                    string tipoProductoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["TipoProductoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int tipoProductoID = CastHelper.CInt2(e.DeleteValues[i].Values["TipoProductoID"]);
                    Cosmos.Compras.Api.Client.TipoProducto.Eliminar(tipoProductoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
