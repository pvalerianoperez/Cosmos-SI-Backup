...
{
    public partial class TipoRepresentanteProveedor_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("TipoRepresentanteProveedor_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("TipoRepresentanteProveedor_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int tipoRepresentanteProveedorID = CastHelper.CInt2(e.InsertValues[i].NewValues["TipoRepresentanteProveedorID"]);
                    ...
                    Cosmos.Compras.Api.Client.TipoRepresentanteProveedor.Guardar(tipoRepresentanteProveedorID, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int tipoRepresentanteProveedorID = CastHelper.CInt2(e.UpdateValues[i].Keys["TipoRepresentanteProveedorID"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int tipoRepresentanteProveedorID = CastHelper.CInt2(e.DeleteValues[i].Values["TipoRepresentanteProveedorID"]);
                    Cosmos.Compras.Api.Client.TipoRepresentanteProveedor.Eliminar(tipoRepresentanteProveedorID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
