...
{
    public partial class Titulo_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Titulo_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Titulo_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int tituloID = CastHelper.CInt2(e.InsertValues[i].NewValues["TituloID"]);
                    ...
                    Cosmos.Compras.Api.Client.Titulo.Guardar(tituloID, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int tituloID = CastHelper.CInt2(e.UpdateValues[i].Keys["TituloID"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int tituloID = CastHelper.CInt2(e.DeleteValues[i].Values["TituloID"]);
                    Cosmos.Compras.Api.Client.Titulo.Eliminar(tituloID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
