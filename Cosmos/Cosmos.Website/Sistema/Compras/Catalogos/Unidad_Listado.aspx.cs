...
{
    public partial class Unidad_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Unidad_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Unidad_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string unidadClave = CastHelper.CStr2(e.InsertValues[i].NewValues["UnidadClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Unidad.Guardar(unidadID, unidadClave, nombre, nombreCorto, estatus);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int unidadID = CastHelper.CInt2(e.UpdateValues[i].Keys["UnidadID"]);
                    string unidadClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["UnidadClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int unidadID = CastHelper.CInt2(e.DeleteValues[i].Values["UnidadID"]);
                    Cosmos.Compras.Api.Client.Unidad.Eliminar(unidadID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
