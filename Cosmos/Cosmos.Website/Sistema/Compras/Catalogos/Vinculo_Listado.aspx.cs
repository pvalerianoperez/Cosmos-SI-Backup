...
{
    public partial class Vinculo_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Vinculo_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Vinculo_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int vinculoID = CastHelper.CInt2(e.InsertValues[i].NewValues["VinculoID"]);
                    string vinculoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["VinculoClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Vinculo.Guardar(vinculoID, vinculoClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int vinculoID = CastHelper.CInt2(e.UpdateValues[i].Keys["VinculoID"]);
                    string vinculoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["VinculoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int vinculoID = CastHelper.CInt2(e.DeleteValues[i].Values["VinculoID"]);
                    Cosmos.Compras.Api.Client.Vinculo.Eliminar(vinculoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
