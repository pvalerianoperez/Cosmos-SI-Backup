...
{
    public partial class Zona_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Zona_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Zona_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int zonaID = CastHelper.CInt2(e.InsertValues[i].NewValues["ZonaID"]);
                    string zonaClave = CastHelper.CStr2(e.InsertValues[i].NewValues["ZonaClave"]);
                    ...
                    Cosmos.Compras.Api.Client.Zona.Guardar(zonaID, zonaClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int zonaID = CastHelper.CInt2(e.UpdateValues[i].Keys["ZonaID"]);
                    string zonaClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["ZonaClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int zonaID = CastHelper.CInt2(e.DeleteValues[i].Values["ZonaID"]);
                    Cosmos.Compras.Api.Client.Zona.Eliminar(zonaID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
