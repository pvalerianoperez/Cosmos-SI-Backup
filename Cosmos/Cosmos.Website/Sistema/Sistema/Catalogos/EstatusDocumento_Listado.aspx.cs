...
{
    public partial class EstatusDocumento_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("SistemaEstatusDocumento_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("SistemaEstatusDocumento_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string sistemaEstatusDocumentoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["SistemaEstatusDocumentoClave"]);
                    ...
                    Cosmos.Sistema.Api.Client.EstatusDocumento.Guardar(sistemaEstatusDocumentoID, sistemaEstatusDocumentoClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int sistemaEstatusDocumentoID = CastHelper.CInt2(e.UpdateValues[i].Keys["SistemaEstatusDocumentoID"]);
                    string sistemaEstatusDocumentoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["SistemaEstatusDocumentoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int sistemaEstatusDocumentoID = CastHelper.CInt2(e.DeleteValues[i].Values["SistemaEstatusDocumentoID"]);
                    Cosmos.Sistema.Api.Client.EstatusDocumento.Eliminar(sistemaEstatusDocumentoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
