...
{
    public partial class EstatusProducto_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("SistemaEstatusProducto_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("SistemaEstatusProducto_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string estatusProductoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["EstatusProductoClave"]);
                    ...
                    Cosmos.Sistema.Api.Client.EstatusProducto.Guardar(estatusProductoID, estatusProductoClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int estatusProductoID = CastHelper.CInt2(e.UpdateValues[i].Keys["EstatusProductoID"]);
                    string estatusProductoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["EstatusProductoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int estatusProductoID = CastHelper.CInt2(e.DeleteValues[i].Values["EstatusProductoID"]);
                    Cosmos.Sistema.Api.Client.EstatusProducto.Eliminar(estatusProductoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
