...
{
    public partial class EstatusRegla_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("SistemaEstatusRegla_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("SistemaEstatusRegla_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int estatusReglaID = CastHelper.CInt2(e.InsertValues[i].NewValues["EstatusReglaID"]);
                    ...
                    Cosmos.Sistema.Api.Client.EstatusRegla.Guardar(estatusReglaID, sistemaEstatusTipoDocumentoID, sistemaEstatusTipoDocumentoIDPermite);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int estatusReglaID = CastHelper.CInt2(e.UpdateValues[i].Keys["EstatusReglaID"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int estatusReglaID = CastHelper.CInt2(e.DeleteValues[i].Values["EstatusReglaID"]);
                    Cosmos.Sistema.Api.Client.EstatusRegla.Eliminar(estatusReglaID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
