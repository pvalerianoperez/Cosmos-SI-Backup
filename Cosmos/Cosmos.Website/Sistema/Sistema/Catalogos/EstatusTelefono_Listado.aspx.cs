...
{
    public partial class EstatusTelefono_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("SistemaEstatusTelefono_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("SistemaEstatusTelefono_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                ...
                {
                    int estatusTelefonoID = CastHelper.CInt2(e.InsertValues[i].NewValues["EstatusTelefonoID"]);
                    string estatusTelefonoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["EstatusTelefonoClave"]);
                    ...
                    Cosmos.Sistema.Api.Client.EstatusTelefono.Guardar(estatusTelefonoID, estatusTelefonoClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int estatusTelefonoID = CastHelper.CInt2(e.UpdateValues[i].Keys["EstatusTelefonoID"]);
                    string estatusTelefonoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["EstatusTelefonoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int estatusTelefonoID = CastHelper.CInt2(e.DeleteValues[i].Values["EstatusTelefonoID"]);
                    Cosmos.Sistema.Api.Client.EstatusTelefono.Eliminar(estatusTelefonoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
