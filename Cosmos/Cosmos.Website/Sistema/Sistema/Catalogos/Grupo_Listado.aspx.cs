...
{
    public partial class Grupo_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("SistemaGrupo_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("SistemaGrupo_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    Cosmos.Sistema.Api.Client.Grupo.Guardar(sistemaGrupoID, nombre, moduloID, activo);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int sistemaGrupoID = CastHelper.CInt2(e.UpdateValues[i].Keys["SistemaGrupoID"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int sistemaGrupoID = CastHelper.CInt2(e.DeleteValues[i].Values["SistemaGrupoID"]);
                    Cosmos.Sistema.Api.Client.Grupo.Eliminar(sistemaGrupoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
