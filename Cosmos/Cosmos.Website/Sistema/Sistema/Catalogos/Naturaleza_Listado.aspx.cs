...
{
    public partial class Naturaleza_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("SistemaNaturaleza_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("SistemaNaturaleza_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string naturalezaClave = CastHelper.CStr2(e.InsertValues[i].NewValues["NaturalezaClave"]);
                    ...
                    Cosmos.Sistema.Api.Client.Naturaleza.Guardar(naturalezaID, naturalezaClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int naturalezaID = CastHelper.CInt2(e.UpdateValues[i].Keys["NaturalezaID"]);
                    string naturalezaClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["NaturalezaClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int naturalezaID = CastHelper.CInt2(e.DeleteValues[i].Values["NaturalezaID"]);
                    Cosmos.Sistema.Api.Client.Naturaleza.Eliminar(naturalezaID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
