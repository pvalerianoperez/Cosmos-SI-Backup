...
{
    public partial class Sexo_Listado : System.Web.UI.Page
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (accion) {
                case "EXPORTAR_EXCEL":
                    ...
                    ASPxGridViewExporter1.WriteXlsxToResponse(string.Format("Sexo_{0}.xlsx", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
                case "EXPORTAR_PDF":
                    ASPxGridViewExporter1.DataBind();
                    ASPxGridViewExporter1.WritePdfToResponse(string.Format("Sexo_{0}.pdf", DateTime.Now.ToString("yyMMddHHmmss")), true);
                    break;
            } 
        ...
        protected void Grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("AGREGAR"))
            {
                for (int i = 0; i < e.InsertValues.Count; i++)
                {
                    ...
                    string sexoClave = CastHelper.CStr2(e.InsertValues[i].NewValues["SexoClave"]);
                    ...
                    Cosmos.Sistema.Api.Client.Sexo.Guardar(sexoID, sexoClave, nombre, nombreCorto);
                }
            }
            //update
            if (((SitioProtegido)this.Master).PermisoAccion("MODIFICAR"))
            {
                ...
                {
                    int sexoID = CastHelper.CInt2(e.UpdateValues[i].Keys["SexoID"]);
                    string sexoClave = CastHelper.CStr2(e.UpdateValues[i].NewValues["SexoClave"]);
            ...
            if (((SitioProtegido)this.Master).PermisoAccion("ELIMINAR"))
            {
                ...
                {
                    int sexoID = CastHelper.CInt2(e.DeleteValues[i].Values["SexoID"]);
                    Cosmos.Sistema.Api.Client.Sexo.Eliminar(sexoID);
                }
            }
            e.Handled = true;
            Grid.DataBind();
